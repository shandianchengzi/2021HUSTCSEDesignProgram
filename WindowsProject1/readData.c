...
Status read_Data(FILE *fp,hash_struct **stations,lineInfo* lineDetail){
    ...
    for(k=0; k<linenum; k++)
    {
        ...
        //printf("%d号线，满员%d人，长度%.2f千米，有%d个站点，线路权重是%f\n",lineDetail[k].lineIndex,lineDetail[k].lineFull,lineDetail[k].lineLength,lineDetail[k].lineCount,lineDetail[k].lineAverage);
